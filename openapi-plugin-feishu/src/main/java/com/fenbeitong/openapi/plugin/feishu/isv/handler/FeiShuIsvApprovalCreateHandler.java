...
@Component
@Slf4j
public class FeiShuIsvApprovalCreateHandler implements ITaskHandler {
    ...
    public TaskType getTaskType() {
        return TaskType.FEISHU_ISV_APPROVAL_CREATE;
    }
    ...
    public TaskResult execute(Task task) throws Exception {
        ...
        if (processType == SaasApplyType.ChaiLv.getValue()) {
            ...
            List<Map> tripMapList = JsonUtils.toObj(form, new TypeReference<List<Map>>() {});
            FeiShuApprovalFormDTO.Value feiShuApprovalFormDTO = null;
            for (Map formMap : tripMapList) {
            ...
            commonApplyReqDTO = feiShuIsvApprovalService.parseFeiShuCarApprovalForm(dataId, list , feishuIsvCompany.getCompanyId(), userId);
            ...
            return processApply.processApply( task  , status );
        }
