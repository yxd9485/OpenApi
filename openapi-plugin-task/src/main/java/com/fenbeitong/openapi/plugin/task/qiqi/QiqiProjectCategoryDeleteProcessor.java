...
import com.fenbeitong.openapi.plugin.support.project.dto.DeleteProjectGroupReqDTO;
...
/**
 * @ClassName QiqiProjectCategoryDeleteProcessor
 * @Description
...
@Component
@Slf4j
public class QiqiProjectCategoryDeleteProcessor implements ITaskProcessor {
    @Autowired
    ...
    public Integer getTaskType() {
        return TaskType.QIQI_SYNC_PROJECT_CATEGORY_DELETE.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) throws Exception {
        ...
        //3.转换数据
        List<DeleteProjectGroupReqDTO> projectGroupDTOS = Lists.newArrayList();
        DeleteProjectGroupReqDTO groupDTO = DeleteProjectGroupReqDTO.builder().thirdGroupId(dataId).build();
        projectGroupDTOS.add(groupDTO);
        //4.同步
        cacheProjectGroupService.deleteProjectGroupBatch(OpenType.QIQI.getType(), corpInfo.getCompanyId(), projectGroupDTOS);
        log.info("【qiqi】 QiqiProjectCategoryDeleteProcessor, 项目分组删除成功,dataId:{}", dataId);
