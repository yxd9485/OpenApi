...
/**
 * @ClassName QiqiRankUpdateProcessor
 * @Description
...
@Component
@Slf4j
public class QiqiRankUpdateProcessor implements ITaskProcessor {
    @Autowired
    ...
    public Integer getTaskType() {
        return TaskType.QIQI_SYNC_RANK_UPDATE.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) throws Exception {
        ...
        if (CollectionUtils.isBlank(qiqiRankReqDTOS)) {
            log.info("【qiqi】 QiqiRankUpdateProcessor, 查询三方数据为空");
        ...
        //5.同步
        cacheEmployeeRankTemplateService.updateRankBatch(OpenType.QIQI.getType(), corpInfo.getCompanyId(), optRankDTOList);
