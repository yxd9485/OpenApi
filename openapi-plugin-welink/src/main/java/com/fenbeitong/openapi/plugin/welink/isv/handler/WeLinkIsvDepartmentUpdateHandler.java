...
@Service
@Slf4j
public class WeLinkIsvDepartmentUpdateHandler implements ITaskHandler {
    @Autowired
    ...
    public TaskType getTaskType() {
        return TaskType.WELINK_ISV_CORP_EDIT_DEPT;
    }
    ...
    public TaskResult execute(Task task) {
        ...
        if (srcOrgUnitList == null || srcOrgUnitList.size() == 0) {
            return weLinkIsvDepartmentAddHandler.execute(task);
        }
        //6.转换数据
