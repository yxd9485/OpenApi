...
/**
 * @ClassName QiqiProjectCategoryProcessAddSync
 * @Description
...
@Component
@Slf4j
public class QiqiProjectCategoryProcessAddSync implements QiqiProcessAddSync {
    ...
    public TaskType addSync(String operation, String corpId) throws Exception {
        ...
        if (OperationTypeEnum.CREATE.getCode().equals(operation)) {
            taskType = TaskType.parse(TaskType.QIQI_SYNC_PROJECT_CATEGORY_ADD.getKey());
        } else if (OperationTypeEnum.UPDATE.getCode().equals(operation)) {
            taskType = TaskType.parse(TaskType.QIQI_SYNC_PROJECT_CATEGORY_UPDATE.getKey());
        } else if (OperationTypeEnum.DELETE.getCode().equals(operation)) {
            taskType = TaskType.parse(TaskType.QIQI_SYNC_PROJECT_CATEGORY_DELETE.getKey());
        } else {
    ...
    public void afterPropertiesSet() throws Exception {
        QiqiProcessAddSyncFactory.register(ObjectTypeEnum.PROJECT_CATEGORY.getCode(), this);
    }
}
