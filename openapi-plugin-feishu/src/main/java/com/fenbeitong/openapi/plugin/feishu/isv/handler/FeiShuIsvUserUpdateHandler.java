...
@Component
@Slf4j
public class FeiShuIsvUserUpdateHandler implements ITaskHandler {
    @Autowired
    ...
    @Autowired
    private FeiShuIsvUserAddHandler feiShuIsvUserAddHandler;
    @Autowired
    ...
    public TaskType getTaskType() {
        return TaskType.FEISHU_ISV_UPDATE_USER;
    }
    ...
    public TaskResult execute(Task task) {
        ...
        if (srcEmployeeList == null || srcEmployeeList.size() == 0) {
            return feiShuIsvUserAddHandler.execute(task);
        }
        //6.转换人员
