...
import com.fenbeitong.openapi.plugin.welink.isv.handler.WeLinkIsvUserAddHandler;
...
@Component
@Slf4j
public class WeLinkIsvUserUpdateProcessor extends AbstractTaskProcessor {
    @Autowired
    ...
    public Integer getTaskType() {
        return TaskType.WELINK_ISV_CORP_EDIT_USER.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) {
        ...
        if (srcEmployeeList == null || srcEmployeeList.size() == 0) {
            TaskResult taskResult = weLinkIsvUserAddHandler.execute(FinhubTaskUtils.convert2Task(task));
