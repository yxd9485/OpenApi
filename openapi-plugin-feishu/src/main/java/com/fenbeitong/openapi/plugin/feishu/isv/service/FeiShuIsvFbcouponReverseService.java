...
@Component
@Slf4j
public class FeiShuIsvFbcouponReverseService implements IFeishuIsvProcessApplyService {
    ...
    public TaskResult processApply(Task task , String status)  {
        ...
        if(FeiShuConstant.APPROVAL_INSTANCE_STATUS_APPROVED.equals(status)){
            ...
            fbtOrderApply = commonApplyService.notifyThirdApplyRepulse(corpId, dataId, OpenType.FEISHU_ISV.getType() , ApplyOrderCategory.FB_COUPON.getCategory() , "");
        }
        if (fbtOrderApply) {
