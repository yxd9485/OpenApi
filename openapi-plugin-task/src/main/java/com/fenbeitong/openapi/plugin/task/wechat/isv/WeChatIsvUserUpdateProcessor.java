...
@Component
@Slf4j
public class WeChatIsvUserUpdateProcessor extends AbstractTaskProcessor {
    ...
    @Autowired
    private WeChatIsvUserAddProcessor weChatIsvUserAddHandler;
    ...
    public Integer getTaskType() {
        return TaskType.WECHAT_ISV_UPDATE_USER.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) {
        ...
        try {
            ...
            if (e.getCode() == NumericUtils.obj2int(WeChatApiResponseCode.WECHAT_ISV_CORP_EMPLOYEE_IS_NULL)) {
                return TaskProcessResult.success("WECHAT_ISV_CORP_EMPLOYEE_IS_NULL success");
            }
            throw e;
        }
        //5.查询分贝通员工信息
        ...
        if (srcEmployeeList == null || srcEmployeeList.size() == 0) {
            return weChatIsvUserAddHandler.process(task);
        }
        //6.拼装分贝通请求参数
        //转换人员
