...
import com.fenbeitong.openapi.plugin.qiqi.constant.EmployeeStatusEnum;
...
/**
 * @ClassName QiqiEmployeeUpdateProcessor
 * @Description
...
@Component
@Slf4j
public class QiqiEmployeeUpdateProcessor implements ITaskProcessor {
    ...
    public Integer getTaskType() {
        return TaskType.QIQI_SYNC_USER_UPDATE.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) throws Exception {
        ...
        if (CollectionUtils.isBlank(employeeInfos)) {
            log.info("【qiqi】 QiqiEmployeeUpdateProcessor, 查询三方数据为空");
        ...
        //5.同步：员工如果是离职状态需要将其删除，否则为修改或新增
        if (EmployeeStatusEnum.RESIGNATION.getCode().equals(employeeList.get(0).getStatus())) {
            openSyncThirdOrgService.deleteEmployee(OpenType.QIQI.getType(), companyId, employeeList);
        } else {
            //查询分贝通员工信息
            List<OpenThirdEmployee> srcEmployeeList = openThirdEmployeeDao.listEmployeeByThirdEmployeeId(OpenType.QIQI.getType(), companyId, Lists.newArrayList(dataId));

            if (CollectionUtils.isBlank(srcEmployeeList)) {
                openSyncThirdOrgService.addEmployee(OpenType.QIQI.getType(), companyId, employeeList);
            } else {
                openSyncThirdOrgService.updateEmployee(OpenType.QIQI.getType(), companyId, employeeList);
            }
        }
        log.info("【qiqi】 QiqiEmployeeUpdateProcessor, 人员修改成功,dataId:{}", dataId);
