...
@Service
@Slf4j
public class WeChatIsvDepartmentAddHandler implements ITaskHandler {
    ...
    @Autowired
    private WeChatIsvDepartmentUpdateHandler weChatIsvDepartmentUpdateHandler;

    @Override
    public TaskType getTaskType() {
        return TaskType.WECHAT_ISV_ORG_DEPT_CREATE;
    }
    ...
    public TaskResult execute(Task task) {
        ...
        if (srcOrgUnitList != null && srcOrgUnitList.size() > 0) {
            return weChatIsvDepartmentUpdateHandler.execute(task);
        }
        //转换部门
