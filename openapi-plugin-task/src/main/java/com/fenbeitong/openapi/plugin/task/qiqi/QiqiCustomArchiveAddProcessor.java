...
/**
 * @ClassName QiqiCustomAchieveAddProcessor
 * @Description
...
@Component
@Slf4j
public class QiqiCustomArchiveAddProcessor implements ITaskProcessor {
    ...
    public Integer getTaskType() {
        return TaskType.QIQI_SYNC_BUDGET_ACCOUNT_ADD.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) throws Exception {
        ...
        if (CollectionUtils.isBlank(customArchiveReqDTOS)) {
            log.info("【qiqi】 QiqiCustomArchiveAddProcessor, 查询三方数据为空");
            //自定义档案只查费用的，查不到直接返回
        ...
        String result = customArchiveProjectService.thirdArchiveProjectCreate(token, companyId, OpenType.QIQI.getType(), archiveReqItemDTO);
        ...
        if (!ObjectUtils.isEmpty(res) && res.success()) {
            log.info("【qiqi】 QiqiCustomArchiveAddProcessor, 自定义档案新增成功,dataId:{}", dataId);
            return TaskProcessResult.success("success");
        } else {
            return TaskProcessResult.fail(JSONObject.toJSONString(res));
        }
    }
