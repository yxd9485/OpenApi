...
import com.fenbeitong.openapi.plugin.event.order.dto.ShansongOrderEvent;
...
public class ShansongOrderEventHandler extends EventHandler<ShansongOrderEvent> {
    ...
    @FuncOrderCallBack(companyId = "companyId", type = 130, status = "status", statusList = {80, 6}, logicKeys = {"orderId", "status"})
    @Override
    public boolean process(ShansongOrderEvent event, Object... args) {
        ...
        Map<String, Object> detail = (Map<String, Object>) funcExpressOrderService.flashSendDetailOrder(expressOrderDetailReqDTO,event.getCompanyId());
        if (!ObjectUtils.isEmpty(detail)) {
            Map orderInfo = (Map) MapUtils.getValueByExpress(detail, "order_info");
            orderInfo.put("order_category_type", 130);
            orderInfo.put("order_category_name", "闪送");
