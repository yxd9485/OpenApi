...
@Service
@Slf4j
public class FeiShuEiaDepartmentUpdateProcessor extends AbstractTaskProcessor {

    @Autowired
    private OpenSyncThirdOrgService openSyncThirdOrgService;

    @Autowired
    private FeiShuEiaDepartmentAddProcessor feiShuEiaDepartmentAddHandler;
    ...
    public Integer getTaskType() {
        return TaskType.FEISHU_EIA_ORG_DEPT_UPDATE.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) {
        ...
        if (srcOrgUnitList == null || srcOrgUnitList.size() == 0) {
            return feiShuEiaDepartmentAddHandler.process(task);
        }
        //6.转换数据
        ...
        //7.同步
        openSyncThirdOrgService.updateDepartment(OpenType.FEISHU_EIA.getType(), companyId, departmentList);
        // 查询配置判断该企业是否要同步部门主管
