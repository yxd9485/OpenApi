...
/**
 * @ClassName QiqiProjectCategoryUpdateProcessor
 * @Description
...
@Component
@Slf4j
public class QiqiProjectCategoryUpdateProcessor implements ITaskProcessor {
    ...
    public Integer getTaskType() {
        return TaskType.QIQI_SYNC_PROJECT_CATEGORY_UPDATE.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) throws Exception {
        ...
        if (CollectionUtils.isBlank(projectGroupDTOS)) {
            log.info("【qiqi】 QiqiProjectCategoryUpdateProcessor, 查询三方数据为空");
        ...
        //5.同步
        cacheProjectGroupService.updateProjectGroupBatch(OpenType.QIQI.getType(), corpInfo.getCompanyId(), groupDTOList);
        log.info("【qiqi】 QiqiProjectCategoryUpdateProcessor, 项目分组修改成功,dataId:{}", dataId);
