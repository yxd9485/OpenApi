...
@Service
@Slf4j
public class FeiShuEiaDepartmentUpdateHandler implements ITaskHandler {
    @Autowired
    private OpenSyncThirdOrgService openSyncThirdOrgService;
    @Autowired
    private FeiShuEiaDepartmentAddHandler feiShuEiaDepartmentAddHandler;
    @Autowired
    ...
    public TaskType getTaskType() {
        return TaskType.FEISHU_EIA_ORG_DEPT_UPDATE;
    }
    ...
    public TaskResult execute(Task task) {
        ...
        if (srcOrgUnitList == null || srcOrgUnitList.size() == 0) {
            return feiShuEiaDepartmentAddHandler.execute(task);
        }
        //6.转换数据
