...
@Component
@Slf4j
public class FeiShuEiaUserAddHandler implements ITaskHandler {
    @Autowired
    private FeiShuEiaEmployeeService feiShuEiaEmployeeService;
    @Autowired
    private OpenSyncThirdOrgService openSyncThirdOrgService;
    @Autowired
    private FeiShuEiaUserUpdateHandler feiShuEiaUserUpdateHandler;
    @Autowired
    ...
    public TaskType getTaskType() {
        return TaskType.FEISHU_EIA_CREATE_USER;
    }
    ...
    public TaskResult execute(Task task) {
        ...
        if (srcEmployeeList != null && srcEmployeeList.size() > 0) {
            return feiShuEiaUserUpdateHandler.execute(task);
        }
        //6.转换人员
