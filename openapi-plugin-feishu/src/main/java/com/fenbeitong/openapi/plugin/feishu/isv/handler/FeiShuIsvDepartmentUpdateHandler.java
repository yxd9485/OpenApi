...
@Service
@Slf4j
public class FeiShuIsvDepartmentUpdateHandler implements ITaskHandler {
    @Autowired
    private FeiShuIsvCompanyDefinitionService feiShuIsvCompanyDefinitionService;
    @Autowired
    private OpenSyncThirdOrgService openSyncThirdOrgService;
    @Autowired
    private FeiShuIsvDepartmentAddHandler feiShuIsvDepartmentAddHandler;
    @Autowired
    ...
    public TaskType getTaskType() {
        return TaskType.FEISHU_ISV_ORG_DEPT_UPDATE;
    }
    ...
    public TaskResult execute(Task task) {
        ...
        if (srcOrgUnitList == null || srcOrgUnitList.size() == 0) {
            return feiShuIsvDepartmentAddHandler.execute(task);
        }
        //6.转换数据
