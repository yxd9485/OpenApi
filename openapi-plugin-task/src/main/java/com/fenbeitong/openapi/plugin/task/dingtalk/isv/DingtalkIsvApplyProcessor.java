...
@ServiceAspect
@Service
public class DingtalkIsvApplyProcessor extends AbstractTaskProcessor {
    ...
    public TaskProcessResult process(FinhubTask task) {
        ...
        DingtalkIsvCompany dingtalkIsvCompany = dingtalkIsvCompanyDao.getDingtalkIsvCompanyByCorpId(corpId);
        ...
        if (apply == null || !dingtalkIsvCompany.getCompanyId().equals(apply.getCompanyId())) {
        ...
        if (!hasBindFbtAccount(thirdUserId, dingtalkIsvCompany.getCompanyId())) {
            ...
            return TaskProcessResult.success("判断钉钉账号未绑定分贝通");
        }
        IDingtalkProcessApplyService processApply = applyFactory.getProcessApply(apply.getProcessType());
        PluginCorpDefinition dingtalkCorp = new PluginCorpDefinition();
        dingtalkCorp.setThirdCorpId(dingtalkIsvCompany.getCorpId());
        dingtalkCorp.setAppId(dingtalkIsvCompany.getCompanyId());
        dingtalkCorp.setThirdAdminId(dingtalkIsvCompany.getThirdAdminId());
        TaskResult result = TaskResult.FAIL;
        try {
            result = processApply.processApply(FinhubTaskUtils.convert2Task(task), dingtalkCorp, apply, processInstanceTopVo);
        } catch (BindException e) {
        }
        return FinhubTaskUtils.convert2FinhubTaskResult(result);
    }
    ...
    public Integer getTaskType() {
        return TaskType.DINGTALK_ISV_BPMS_INSTANCE_CHANGE.getCode();
    }
