...
@Component
@Slf4j
public class HyprocaTripApplyCreateProcessor extends AbstractTripApplyService implements ITaskProcessor {
    ...
    public Integer getTaskType() {
        return TaskType.HYPROCA_TRIP_APPLY_CREATE.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) {
        ...
        if (pluginCorpDefinition == null) {
            ...
            return TaskProcessResult.success("企业不存在，任务丢弃 sucess");
        }
        ...
        // 封装数据
        CommonApplyReqDTO commonApplyReqDTO = setData(thipApplyDto,corpId);
        try {
    ...
    public CommonApplyReqDTO setData(HyprocaTripApplyDto thipApplyDto,String companyId) {
        ...
        String thirdId = StringUtils.isEmpty(thipApplyDto.getTripId())?thipApplyDto.getWfinstanceId():thipApplyDto.getTripId();
