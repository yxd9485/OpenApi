...
@Component
@Slf4j
public class WeChatIsvUserAddHandler implements ITaskHandler {
    @Autowired
    ...
    @Autowired
    private WeChatIsvUserUpdateHandler weChatIsvUserUpdateHandler;
    @Autowired
    ...
    public TaskType getTaskType() {
        return TaskType.WECHAT_ISV_CREATE_USER;
    }
    ...
    public TaskResult execute(Task task) {
        ...
        if (srcEmployeeList != null && srcEmployeeList.size() > 0) {
            return weChatIsvUserUpdateHandler.execute(task);
        }
        //6.拼装分贝通请求参数
        //转换人员
