...
@Service
@Slf4j
public class FeiShuIsvDepartmentAddProcessor extends AbstractTaskProcessor {
    @Autowired
    ...
    public Integer getTaskType() {
        return TaskType.FEISHU_ISV_ORG_DEPT_CREATE.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) {
        ...
        if (srcOrgUnitList != null && srcOrgUnitList.size() > 0) {
            return feiShuIsvDepartmentUpdateHandler.process(task);
        }
        //6.转换数据
