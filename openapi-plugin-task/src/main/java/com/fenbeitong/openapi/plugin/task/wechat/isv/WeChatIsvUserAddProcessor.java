...
@Component
@Slf4j
public class WeChatIsvUserAddProcessor extends AbstractTaskProcessor {
    @Autowired
    ...
    @Autowired
    private WeChatIsvUserUpdateProcessor weChatIsvUserUpdateHandler;
    @Autowired
    ...
    public Integer getTaskType() {
        return TaskType.WECHAT_ISV_CREATE_USER.getCode();
    }
    ...
    public TaskProcessResult process(FinhubTask task) {
        ...
        try {
            ...
            if (e.getCode() == NumericUtils.obj2int(WeChatApiResponseCode.WECHAT_ISV_CORP_EMPLOYEE_IS_NULL)) {
                //4.如果在企业微信查不到，标记为已处理
                return TaskProcessResult.success("如果在企业微信查不到，标记为已处理 success");
            }
            throw e;
        }
        //4.查询分贝通员工信息
        ...
        if (srcEmployeeList != null && srcEmployeeList.size() > 0) {
            return weChatIsvUserUpdateHandler.process(task);
        }
        //6.拼装分贝通请求参数
        //转换人员
        ...
        openThirdEmployeeDTO.setThirdEmployeeName(WeChatIsvConstant.WECHAT_ISV_USER_NAME);
        ...
        openSyncThirdOrgService.addEmployee(OpenType.WECHAT_ISV.getType(), companyId, employeeList);
